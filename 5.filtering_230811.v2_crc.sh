...
process_sample() {
    ...
    awk -F'\t' -v OFS='\t' 'NR > 1 { print $2 }' "${output_path}/${sample}.filtered.txt" > "${output_path}/${sample}.read1.txt"
    awk -F'\t' -v OFS='\t' 'NR > 1 { print $2 }' "${output_path}/${sample}.filtered.txt" > "${output_path}/${sample}.read2.txt"
