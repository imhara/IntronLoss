...
## sample name setting 									-- updated at 2024.01.22
...
bwa mem -M -R $RG $ref_dir/$gene_ref $fastq_dir/$fastq1 $fastq_dir/$fastq2 -t $thread > $output_dir/$output_prefix/${output_prefix}.${panel}.R3.sam | tee -a $output_dir/$output_prefix/${output_prefix}_log.txt
...
rm -rf ${output_dir}/${output_prefix}/*log.txt 
